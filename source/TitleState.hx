...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		add(teamPortrait);
		teamPortrait.visible = false;
		teamPortrait.setGraphicSize(Std.int(teamPortrait.width * 0.8));
		teamPortrait.updateHitbox();
	...
	override function beatHit()
	{
		...
		if (danceLeft)
			soupDance.animation.play('danceRight');
		else
		...
		switch (curBeat)
		{
			...
			case 5:
				if (Main.watermarks)
					createCoolText(['A Mod Developed', 'by']);
				else
					createCoolText(['In Partnership', 'with']);
			case 7:
				if (Main.watermarks)
				{
					addMoreText('Meeper Creepertown');
					teamPortrait.visible = true;
				}
				else
				{
